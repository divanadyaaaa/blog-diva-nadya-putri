                ...
                <form method="POST" action="{{ route('categories.store') }}">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="border-gray-200 rounded-xl shadow-sm focus:border-primary-400 focus:ring focus:ring-primary-100 focus:ring-opacity-50 transition w-full">{{ old('description') }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
