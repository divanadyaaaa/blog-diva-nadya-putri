                ...
                <form method="POST" action="{{ route('categories.update', $category->id) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="border-gray-300 rounded-md shadow-sm focus:border-primary-400 focus:ring focus:ring-primary-200 focus:ring-opacity-50 transition w-full mt-1">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
